...
let conn = new WebSocket('ws://127.0.0.1:8881');
...
function newMessagePopup(_this) {
    ...
    let userIdRecever = $(_this).attr('rel').replace('user_', '');
    console.log('message: ' + message + 'to: ' + userIdRecever + ' from: ' + id);
    conn.send(JSON.stringify({command: "privateMessage", roomId: roomId, to: userIdRecever, message: message}));
